...
- Story: — (Wave 10 2/6 DONE — FIX-246 closed; FIX-235 next)
- Step: —
- Last closed: FIX-246 [x] DONE 2026-04-27 (Wave 10 P2 M — Quotas + Resources merge → Tenant Usage; F-271/272/273/274/314/315/316/317; new /admin/tenant-usage page + tenant-usage-card + tenant-usage-detail-panel + quota-bar 4-tier color (success/warning/danger/critical+pulse) + useTenantUsage 30s refetch + ListTenantUsage handler GET /api/v1/admin/tenants/usage + quota_breach_checker @hourly cron with FIX-209 alert reuse (Source='system' per CHECK constraint, dedup quota:{tenant}:{metric}:{severity}, severity medium/critical FIX-211, auto-resolve below 80%) + EstimateAPIRPS proxy + Tenant struct +Plan +MaxSessions +MaxAPIRPS +MaxStorageBytes (PAT-006 audit, 5 SELECT/Scan pairs) + migration 20260502000001 plan column + 3 quota cols + GREATEST conservative + seed 009 idempotent tenant_admin orphan fix + 2 old routes 301-redirect + sidebar Quotas+Resources → Tenant Usage; CRON_QUOTA_BREACH_CHECK env; 12 ACs (9 PASS + 1 CRITICAL fixed F-A1 alert source CHECK + 2 HIGH fixed + 4 MEDIUM fixed + 9 LOW fixed); 7 USERTEST scenarios; PAT-024 NEW (fake stores hide CHECK constraint violations); DEV-566..570; D-170 (7-day trend endpoint) + D-171 (recent_breaches type field) deferred + D-153/D-156 re-routed)
- Wave 10 P2 IN PROGRESS (2/6 — FIX-240 + FIX-246 DONE; FIX-235/245/238/247 PENDING)
