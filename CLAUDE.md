...
- Story: — (Wave 10 3/6 DONE — FIX-235 closed; FIX-245 next)
- Step: —
- Last closed: FIX-235 [x] DONE 2026-04-27 (Wave 10 P2 XL — M2M eSIM Provisioning Pipeline SGP.22→SGP.02 + SM-SR + Bulk; F-172/173/174/175/176/178/180/181/182/184; 3 migrations (esim_ota_commands + esim_profile_stock + state CHECK) + new internal/smsr/ pkg (Client interface + Mock with FailRate) + EsimOTACommandStore (10 methods state-machine UPDATE WHERE) + EsimProfileStockStore (atomic UPDATE WHERE available>0, ErrStockExhausted) + 4 workers (esim_ota_dispatcher rate-limited per-operator + exp backoff 30/60/120/240/480s + 5x max + audit + bus emit; esim_ota_timeout_reaper */2 cron sent>10min; bulk_esim_switch refactored batch INSERT no N+1; esim_stock_alert */15 cron PAT-024 Source="system" with regression test) + 4 endpoints (BulkSwitch + OTACallback HMAC-SHA256 X-SMSR-Signature 300s replay window + StockSummary + OTAHistory) + 9 env vars (SMSR_CALLBACK_SECRET prod-required, MOCK_SMSR_FAIL_RATE, ESIM_OTA_RATE_LIMIT_PER_SEC=100, ESIM_OTA_BATCH_SIZE=200, ESIM_OTA_MAX_RETRIES=5, ESIM_OTA_TIMEOUT_MINUTES=10, CRON_ESIM_OTA_DISPATCHER, CRON_ESIM_OTA_TIMEOUT_REAPER, CRON_ESIM_STOCK_ALERT) + FE redesign: eSIM list bulk bar + EID mask formatEID + ICCID Link + state filter +failed; Operator Detail eSIM Profiles tab between SIMs and Audit (2 StatCards Stock + State Breakdown); SIM Detail eSIM card masked EID + ProfileHistoryPanel + AllocateFromStockPanel SlidePanel; PROTOCOLS.md SGP.02 chapter (~130 lines); ARCHITECTURE.md eSIM M2M section; 12 ACs (8 PASS + 1 CRITICAL F-U1 dead button removed + 5 HIGH fixed F-A1 EID/ICCID confusion runtime bug + F-A2/F-U2 33 px violations → tokens + F-A3 PAT-017 Config plumbing + F-U3 mobile sticky deferred D-175 + F-A5 eids selector handler-rejection + F-A6 rejected callback audit); 10 USERTEST scenarios; PAT-025 NEW (EID/ICCID semantic column confusion — fakes hide it, integration test asserting EID column value mandatory); DEV-571..574; 8 tech debt routed D-172..D-179; 3896 Go tests pass; 53 files +1768/-295 net)
- Wave 10 P2 IN PROGRESS (3/6 — FIX-240 + FIX-246 + FIX-235 DONE; FIX-245/238/247 PENDING)
