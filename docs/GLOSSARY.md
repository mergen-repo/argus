...
| Job Runner | NATS JetStream-backed background job processor. Subscribes to `argus.jobs.queue`, locks jobs (`locked_by` worker ID), dispatches to registered processors, publishes progress and completion events. Features: distributed Redis locking (Lua-script atomic release/renew), per-tenant concurrency control (default 5), lock TTL auto-renewal during execution, cron scheduler with Redis dedup for multi-instance safety, timeout detector (30min auto-fail for stale jobs), graceful cancellation via context propagation, retry via new job creation. 11 job type constants with registered processors. | SVC-09, STORY-013, STORY-031, F-068 |
