...
| Row-Level Security (RLS) | PostgreSQL feature that restricts which rows a database role can access. In Argus, RLS is enabled with `FORCE ROW LEVEL SECURITY` on 28 tenant-scoped tables using the session variable `app.current_tenant`. The application role uses `BYPASSRLS` (defense-in-depth pattern — app-layer scoping remains the primary isolation boundary). Policies are defined in `migrations/20260412000006_rls_policies`. See [docs/architecture/db/rls.md](architecture/db/rls.md). | Multi-tenant isolation, STORY-064, DEV-167, TBL-01..TBL-31 |
