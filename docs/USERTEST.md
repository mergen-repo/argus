...
## STORY-071: Roaming Agreement Management — REMOVED (FIX-238, 2026-04-30)

The roaming agreements feature (UI, API, DB table TBL-43, DSL `roaming` field,
`roaming.agreement.renewal_due` event, `ROAMING_RENEWAL_*` env vars) was
removed by FIX-238. SCR-150 / SCR-151 are retired; API-230..235 are retired;
`/roaming-agreements` returns 404; `/operators/:id?tab=agreements` aliases to
the Overview tab. Steering of Roaming (SoR) network selection (STORY-026) is
unaffected. No user-test scenarios remain for this story.

---

## FIX-238: Remove Roaming Feature — Removal Verification

### Scenario 1 — /roaming-agreements returns 404

1. Navigate to `http://localhost:8084/roaming-agreements`.
2. **Expected:** Browser shows 404 / "Not Found" page (React router fallback or Nginx 404). No roaming list UI.
3. Navigate to `http://localhost:8084/roaming-agreements/some-uuid`.
4. **Expected:** Same 404. No detail page.

### Scenario 2 — Sidebar has no Roaming link

1. Log in as `admin@argus.io`.
2. Inspect left sidebar OPERATIONS section.
3. **Expected:** No "Roaming" or "Roaming Agreements" link visible. Sidebar items: SIMs, Sessions, Policies, Alerts, Violations (no Roaming).

### Scenario 3 — Operator Detail ?tab=agreements redirects to Overview

1. Navigate to `http://localhost:8084/operators/<any-operator-id>?tab=agreements`.
2. **Expected:** Page loads with Overview tab active (URL updates to `?tab=overview` or tab=agreements silently maps to overview content). No "Agreements" tab in the tab bar.
3. Verify tab bar shows: Overview, Protocols, Health, Traffic, Sessions, SIMs, eSIM, Alerts, Audit (no Agreements tab).

### Scenario 4 — AC-10: Boot-time roaming keyword archiver

1. Seed a policy version with `roaming` in its DSL content (state=`active`) in the DB:
   ```sql
   INSERT INTO policy_versions (id, policy_id, version_num, dsl_content, compiled_rules, state)
   VALUES (gen_random_uuid(), '<existing-policy-id>', 99,
           'POLICY "test" { MATCH { roaming = true } RULES { bandwidth_down = 1mbps } }',
           '{}', 'active');
   ```
2. Restart the Argus service (`make down && make up` or `docker compose restart argus`).
3. **Expected:** After startup, the seeded version's state is `archived` (not `active`). Audit log contains an entry: `action=policy_version.archived`, `reason` contains "roaming keyword".
4. Re-run restart — **Expected:** archiver runs again idempotently, count=0 additional archives.
...
2. **Beklenen:** Açılır listede `apn`, `imsi`, `tenant`, `msisdn`, `rat_type`, `sim_type`, `operator`, `group` görünür (vocab endpoint'inden gelir; `roaming` FIX-238 ile kaldırıldı).
