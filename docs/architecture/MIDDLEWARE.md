...
- **Bulk SIM endpoints** (`/sims/bulk/state-change`, `/sims/bulk/policy-assign`, `/sims/bulk/operator-switch`) are rate-limited at **1 req/sec per tenant (burst 2)** via the `BulkRateLimiter` middleware (`internal/gateway/bulk_ratelimit.go`), wired into the chi router via `r.With(bulkRL)`. This is stricter than the default per-minute tenant limit. Exceeding this limit returns `429 RATE_LIMITED`. See [api/bulk-actions.md](api/bulk-actions.md) for full endpoint specifications.
