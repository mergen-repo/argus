...
| SVC-09 (Job Runner) | Quota Breach Checker | `internal/job/quota_breach_checker.go` | Hourly cron (`@hourly`, override via `CRON_QUOTA_BREACH_CHECK`); queries `admin_tenant_usage` view; emits `alert.kind = quota.breach` at 80% (warning) and 95% (critical) per metric (SIMs, sessions, API RPS, storage); auto-resolves alerts when metric drops back below 80%. FIX-246. |
