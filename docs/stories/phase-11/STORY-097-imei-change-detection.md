...
- [ ] AC-5: Notification `imei.changed` is published whenever the enforcer (STORY-096) detects mismatch (excluding `BLACKLIST` which uses `device.binding_blacklist_hit` and `grace-period` mid-window accepted changes which use `device.binding_grace_change`). Severity is scaled by binding_mode and mismatch type: `strict` mismatch, `allowlist` mismatch, and `grace-period-expired` (post-expiry reject) → `high`; `tac-lock` mismatch and `grace-period` mid-window AllowWithAlarm → `medium`; `soft` mismatch and `first-use` lock event → `info`. `BLACKLIST` hit (separate subject) → `high`. Subject and payload conform to the `bus.Envelope` v2 contract (FIX-212). The corresponding `imei_history` row has `alarm_raised=true`. Code is canonical for the severity table; see VAL-066 for the `grace-period-expired=high` reconciliation.
