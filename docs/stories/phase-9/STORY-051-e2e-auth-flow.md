...
## Acceptance Criteria
- [ ] Test 1 — Login: POST /api/v1/auth/login → JWT obtained
- [ ] Test 2 — Dashboard: GET /api/v1/analytics/dashboard → 200, metrics present
- [ ] Test 3 — Create APN: POST /api/v1/apns → 201, APN with IP pool created
- [ ] Test 4 — Bulk Import: POST /api/v1/sims/bulk/import (CSV with 10 SIMs) → 202, job created
- [ ] Test 5 — Wait for job: poll GET /api/v1/jobs/:id until state=completed
- [ ] Test 6 — Verify SIMs: GET /api/v1/sims → 10 SIMs in active state, IPs assigned
- [ ] Test 7 — Create Policy: POST /api/v1/policies → policy with DSL rules created
- [ ] Test 8 — Activate Policy: POST /api/v1/policy-versions/:id/activate → policy active
- [ ] Test 9 — Assign Policy: POST /api/v1/sims/bulk/policy-assign → policy assigned to SIMs
- [ ] Test 10 — RADIUS Auth: send Access-Request for imported SIM → Access-Accept with policy QoS
- [ ] Test 11 — Verify Session: GET /api/v1/sessions → session for authenticated SIM exists
- [ ] Test 12 — WebSocket: connect to WS, verify session.started event received
- [ ] All tests run in isolated Docker environment (docker compose up)
- [ ] Test cleanup: teardown created resources after test suite
- [ ] Test runs in CI pipeline (GitHub Actions)
